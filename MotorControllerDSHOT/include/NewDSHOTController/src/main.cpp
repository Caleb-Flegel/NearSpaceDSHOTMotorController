...
void DSHOT150::arm_3d() //USE THIS INSTEAD
{
    Timer t;
        ...
        while (t.elapsed_time() < 7s) {
            throttle_3d(0);         //gives throttle signal of 0% which is 1048
                                    //step 2/2 of arming sequence is for ESC to read 0 rpm
        }
}
...
int main() //3d mode
{
    ...
    t.start(); 
    while (t.elapsed_time() < 5s && thrust < .5){
      motor.throttle_3d(thrust += .0005);
    }
  
    while (t.elapsed_time() < 10s && thrust > -.5){
      motor.throttle_3d(thrust -= .001);
    }
    while (t.elapsed_time() < 15s && thrust < 0){
      motor.throttle_3d(thrust += .0005);
    }
